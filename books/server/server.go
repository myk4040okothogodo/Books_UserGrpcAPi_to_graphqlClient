...
func (s *Server) GetBook(ctx context.Context, in *booksv1.GetBookRequest)(*booksv1.GetBookResponse, error){
    ...
    if err != nil {
        if driver.IsNotFound(err){
            ...
            err = fmt.Errorf("Failed to get book with id '%s'", in.Id, err)
        }
        return nil, err
    }
    book.Id = meta.Key
...
func (s *Server) GetBooks(ctx context.Context, in *booksv1.GetBooksRequest) (*booksv1.GetBooksResponse, error){
    if in == nil || len(in.Ids) == 0 {
        return nil, fmt.Errorf("Books ids are not provided")
    }
    ...
    if err != nil {
        return nil, fmt.Errorf("Failed to iterate over book docments with query '%s': %s", queryBooksByIds, err)
    }
